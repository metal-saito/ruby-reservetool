...
- 新しいサンプルディレクトリを追加する場合は `samples/XX_topic_name` の命名で、`README` とテストを必ず含めてください。
- Lint / Format には RuboCop を任意導入可。`bundle exec rubocop` が通ることを推奨します。
- 追加サンプルには最小限でも 1 つの自動テストと、背景・使用方法・設計ポイントの記載を含めてください。
